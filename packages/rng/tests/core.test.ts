...
describe('hmac', () => {
  ...
  it('matches every committed test vector', () => {
    for (const v of vectors) {
      ...
      expect(out.toString('hex')).toBe(v.hmacHex);
    }
  });
  ...
  it('changes when any input changes', () => {
    const base = hmac('0'.repeat(64), 'x', 0, 0).toString('hex');
    expect(hmac('1' + '0'.repeat(63), 'x', 0, 0).toString('hex')).not.toBe(base);
    expect(hmac('0'.repeat(64), 'y', 0, 0).toString('hex')).not.toBe(base);
    expect(hmac('0'.repeat(64), 'x', 1, 0).toString('hex')).not.toBe(base);
    expect(hmac('0'.repeat(64), 'x', 0, 1).toString('hex')).not.toBe(base);
  });
});
...
describe('deriveFloat', () => {
  it('matches the vector floats for all committed vectors', () => {
    for (const v of vectors) {
      const buf = Buffer.from(v.hmacHex, 'hex');
  ...
  it('rejects wrong-length buffers', () => {
    expect(() => deriveFloat(Buffer.alloc(31), 0)).toThrow();
    expect(() => deriveFloat(Buffer.alloc(33), 0)).toThrow();
  });

  it('returns 0 for an all-zero hmac', () => {
    expect(deriveFloat(Buffer.alloc(32), 0)).toBe(0);
  });
